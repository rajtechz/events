                            ...
                            return (
                                <Box key={index} onClick={() => navigate("/guestdetail", { state: { attendeeId: item.id } })} sx={(theme) => ({ cursor: "pointer", })} >
                                    <Flex gap={10} sx={() => ({ "@media(max-width:374px)": { flexDirection: "row" } })}>
                                        ...
                                        <Box p={1} type={item?.type} sx={(theme) => ({ background: item?.type === 2 ? "#e98400" : "#1db440", borderRadius: "20px", color: "#fff", fontSize: "18px", fontWeight: "bold" })}><Text mx={10} sx={(theme) => ({ textAlign: "center", })}>{item.type === 2 ? <span>Guest</span> : <span>Awardee</span>}</Text></Box>
